...
func TestMakeThingsDateFilter(t *testing.T) {
	tests := []struct {
		name   string
		column string
		value  any
	...
	}{
		{
			...
			column: "deadline",
			value:  "2021-03-28",
			want:   "AND deadline == 132464128",
		},
		{
			name:   "bool true",
			column: "startDate",
			value:  true,
			want:   "AND startDate IS NOT NULL",
		},
		{
			name:   "bool false",
			column: "startDate",
			value:  false,
			want:   "AND startDate IS NULL",
		},
		{
			name:   "nil",
			column: "startDate",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := makeThingsDateFilter(tt.column, tt.value)
			assert.Equal(t, tt.want, got)
		})
	}
}

func TestMakeUnixTimeFilter(t *testing.T) {
	tests := []struct {
		name   string
		column string
		value  any
	...
	}{
		{
			...
			column: "stopDate",
			value:  "future",
			want:   "AND date(stopDate, 'unixepoch', 'localtime') > date('now', 'localtime')",
		},
		{
			name:   "bool false",
			column: "stopDate",
			value:  false,
			want:   "AND stopDate IS NULL",
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := makeUnixTimeFilter(tt.column, tt.value)
			assert.Equal(t, tt.want, got)
		})
	}
}
