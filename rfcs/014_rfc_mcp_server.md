...
- Pagination on every list/search tool: `limit` and 1-based `page`, returning the `{items, total, page, pages}` envelope. `limit` carries machine-readable schema bounds (default 20, maximum 100) that the SDK applies and validates, so an omitted `limit` is stamped to 20 and an over-cap value is rejected before the handler runs rather than silently clamped. No floor is stamped: a non-positive `limit` or `page` still falls back to the default page, because zero is a common shorthand for "no cap" and answering it beats failing it. There is deliberately no unlimited mode: MCP output lands in a model context window. An out-of-range page returns empty `items` with intact metadata. The default is 20 rather than the CLI's 10 because a tool round trip costs more than a terminal keystroke.
...
- Schema-enforced pagination (above): `limit` carries `default` and `maximum` as real JSON Schema keywords, so the model reads the true cap and the SDK enforces it, replacing a silent server-side clamp the model never saw.
...
- `--max-limit N`: lowers the list page-size cap for the whole session (the default page size is clamped down with it); `0` uses the built-in maximum of 100 and a negative value is rejected outright, so a mistyped cap fails loudly instead of leaving the session uncapped. It can only tighten the cap, never raise it, so a model can never request a larger page than the operator allows.
